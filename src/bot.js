...
const PROMPT_TG = q(`${HEADER}\n\n📱 ${b('Telegram to Number')}\n\nPlease enter a Telegram username or ID.\n\nExample: ${c('@thekittydev')}`);
...
/tginfo ${c('<@username>')} — Look up number from Telegram
...
const LOADER_TG = q(`⏳ ${b('KittyOsint')} is looking up Telegram info...`);
...
function formatTelegram(data) {
  const r = data.result;
  if (!r || !r.number) return q(`${HEADER}\n\n❌ No data found for ${c(data.info)}.`);

  let parts = [HEADER];
  parts.push(`\n┌─── TELEGRAM INFO ───┐`);
  parts.push(fmtLine('📱', 'Number', c(r.number)));
  if (r.country) parts.push(fmtLine('🌍', 'Country', fval(r.country)));
  if (r.country_code) parts.push(fmtLine('📞', 'Code', fval(r.country_code)));
  if (r.tg_id) parts.push(fmtLine('🆔', 'TG ID', c(r.tg_id)));
  parts.push(`└${'─'.repeat(22)}┘`);
  parts.push(`\n┌─── USAGE ───┐`);
  if (r.daily_usage) parts.push(fmtLine('📊', 'Daily', fval(r.daily_usage)));
  if (r.req_left) parts.push(fmtLine('💡', 'Left', c(r.req_left)));
  if (r.expiry) parts.push(fmtLine('⏳', 'Expiry', fval(r.expiry)));
  parts.push(`└${'─'.repeat(18)}┘`);
...
const KNOWN_COMMANDS = ['start', 'help', 'numinfo', 'aadharinfo', 'pangstinfo', 'vehicleinfo', 'ffinfo', 'tginfo'];
...
function setupBot(bot) {
  ...
  async function processTgLookup(msg, chatId, info) {
    const sent = await bot.sendMessage(chatId, LOADER_TG, { parse_mode: 'HTML', reply_markup: mk(), reply_to_message_id: msg.message_id });
    try {
      const API_URL = process.env.API_URL || `http://localhost:${process.env.PORT || 3000}`;
      const { data } = await axios.get(`${API_URL}/api/tginfo`, { params: { info }, timeout: 20000 });
      if (!data.success || !data.result || !data.result.number) {
        return bot.editMessageText(q(`${HEADER}\n\n❌ No data found for ${c(info)}.`), { chat_id: chatId, message_id: sent.message_id, parse_mode: 'HTML', reply_markup: mk() });
      }
      await bot.editMessageText(formatTelegram(data), { chat_id: chatId, message_id: sent.message_id, parse_mode: 'HTML', reply_markup: mk() });
      sendJSONFile(chatId, bot, data, `${info.replace('@', '')}.json`, msg.message_id);
    } catch {
      await bot.editMessageText(ERR_FAIL, { chat_id: chatId, message_id: sent.message_id, parse_mode: 'HTML', reply_markup: mk() });
    }
  }
...
🎉 ${b('Welcome ' + name + '!')} ${b('KittyOsint')} is ready.`;
  ...
  bot.onText(/\/tginfo(?:\s+(@?\w+))?$/, async (msg, match) => {
    if (!(await requireGroup(msg))) return;
    const chatId = msg.chat.id;
    const info = match[1];
    if (!info) {
      userState.set(msg.from.id, { cmd: 'tginfo', chatId });
      if (!(await requireChannel(bot, msg))) return bot.sendMessage(chatId, JOIN_REQUIRED, { parse_mode: 'HTML', reply_markup: channelKeyboard(), reply_to_message_id: msg.message_id });
      return bot.sendMessage(chatId, PROMPT_TG, { parse_mode: 'HTML', reply_markup: rk(), reply_to_message_id: msg.message_id });
    }
    if (!(await requireChannel(bot, msg))) return bot.sendMessage(chatId, JOIN_REQUIRED, { parse_mode: 'HTML', reply_markup: channelKeyboard(), reply_to_message_id: msg.message_id });
    return processTgLookup(msg, chatId, info);
  });
  ...
  bot.on('message', async (msg) => {
    ...
    if (state && !text.startsWith('/')) {
      ...
      if (state.cmd === 'tginfo') {
        return processTgLookup(msg, chatId, input);
      }
    }
